...
FileName     : AnsiString;  {ðàçìåð ôàéëà - 4; ïîèíòåð íà èìÿ ôàéëà? - 4; ïîèíòåð íà èìÿ ôàéëà ñ ðàñøèðåíèåì-4}
end;
TZar = packed record
//GAR
Size            : LongWord;
...
const
Header : longword = $02524147;
queen : Ansistring = 'jenkins';
end;
   ...
   begin
     ...
     for I := 1 to z.CountFiles do
       begin
         ...
         f.Read(z.FileInfo[i].OffsetNameExt,4);
       end;
     for I := 1 to z.CountFiles do
       begin
         f.Position:=z.FileInfo[i].OffsetNameExt;
     ...
     CloseFile(text);
     Writeln('Done:)');
  end else begin Writeln('Error. Not GAR file'); readln;end;
end else
  begin
    ...
    filename:=ExtractOnlyFileName(ParamStr(1));
    if ParamStr(2)<>'' then
    f:=TFileStream.Create(ParamStr(2)+'.gar',fmcreate) else f:=TFileStream.Create(filename+'.gar',fmcreate);
    f.Write(z.Header,4);
    ...
    for I := 1 to z.CountFilesTypes do
     ...
     for I := 1 to z.CountFilesTypes do
      ...
      f.Position:=f.Position+($C*z.CountFiles);
      for I := 1 to z.CountFiles do
        begin
          z.FileInfo[i].OffsetNameExt:=f.Position;
         ...
         // Seek4(f);
          ...
          ts:=ExtractOnlyFileName(z.FileInfo[i].FileName);
          f.Write(ts[1],length(ts));
          f.Write(bb,1);
          Seek4(f);
        end;
      ...
      for I := 1 to z.CountFiles do
        begin
          ...
          f.Write(z.FileInfo[i].OffsetNameExt,4);
        end;
      f.Position:=$10;
